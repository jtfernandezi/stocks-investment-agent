...
|-----|----------|--------------|---------------|
| **Daily canary** (`canary.mjs`) | Weekdays 21:30 UTC (4:30 PM ET — after market close) | Deterministic integrity check (no LLM). Catches the silent-failure class (cash deadlock, frozen watchlist, partial signals, swallowed query errors) and reconciles **both** `trades` (status=OPEN) **and** `position_metadata` against live Alpaca positions (phantom / untracked / stale-metadata desync). Silent when healthy; **Telegram alarm only on failure.** | No |
